...
class Artscouncil extends CI_Controller {
	var $tablefunc = 'artscouncil';
	...
	function __construct(){
        ...
        $this->categoryarr2 = $this->_formantcat($this->categoryarr);
		$this->recommendarr = $this->Data_model->getData(array('status'=>1,'model'=>$this->tablefunc,'lang'=>$this->editlang),'listorder',0,0,'recommend');
	}

    // 绑定艺术委员会
    private function _formantcat($data)
    {
        $tmp = $data[self::BIND_CATE];
        $this->load->model('Tool_model');
        $result = $this->Tool_model->tree($data,$tmp['id']);
        return $result;
    }
