...
class Review extends CI_Controller {
	...
	function __construct(){
        ...
        $this->categoryarr2 = $this->categoryarr;
        $this->categoryarr2 = $this->Tool_model->tree($this->categoryarr2);
	...
	public function index(){
		...
		if($search['category']>0){
            $thiscategory = $this->Data_model->getData(array('id'=>$search['category']),'',0,0,'category');
            $thiscategory = $thiscategory[0];
            $tmpWhere = array(
                'model'=> 'review',
                'lft >='=>$thiscategory['lft'],
                'lft <'=>$thiscategory['rht'],
            );
            $tmpCategory = $this->Data_model->getData($tmpWhere,'',0,0,'category');
		    $categoryidarr = mult_to_idarr($tmpCategory);
            $getwhere['category']=$categoryidarr;
		}
		...
		$res = array(
                ...
                'categoryarr2'=>$this->categoryarr2,
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
            ...
            $categorystr = '';
            if (isset($this->categoryarr[$item['category']]))
            {
                $pid = $this->categoryarr[$item['category']]['parent'];
                $categorystr = '[<a href="'.site_url('category/'.$this->categoryarr[$pid]['dir']).$this->langurl.'" target="_blank"><font color="green">'.$this->categoryarr[$pid]['name'].'</font></a>]';
            }
